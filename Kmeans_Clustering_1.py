...
line_1 = (0, 0)  # stores the last scatter plot points
...
scatter1 = axa.scatter(X[:, 0], X[:, 1], c=kmeans.predict(X), s=30, cmap='plasma')
...
for i in range(centers.shape[0]):
    ...
    axa.text(centers[i, 0], centers[i, 1], str(i), c='black',
             bbox=dict(boxstyle='round', facecolor='white', edgecolor='black'))
# produce a legend with the unique colors from the scatter
legend1 = axa.legend(*scatter1.legend_elements(), loc="upper right", title="Clusters")
...
def save_plot():
    ...
    fig.savefig('kmeans_clustering_1.png')
...
def set_cluster(value):
    ...
    global kmeans, centers, scatter1, legend_1, legend1, plot_text, result_text, cluster_txt
    ...
    if value > 0:
        ...
        kmeans.fit(X)
        # predict which cluster each data point X belongs to:
        scatter1 = axa.scatter(X[:, 0], X[:, 1], c=kmeans.predict(X), s=50, cmap='plasma')
        # get the array of centroids locations
        centers = kmeans.cluster_centers_
        # loop through all the locations of the clusters
        for i in range(centers.shape[0]):
...
def predict_x1(value):
    # make variables accessible from the outside
    global value_1, value_2, plotCount, line_1, legend_1
    ...
    sample_test = np.array([[value_1, value_2]])
    # plot the scatter plot
    line_1 = axa.scatter(sample_test[0, 0], sample_test[0, 1], c='red', marker='*', s=100, label='Test data point')
